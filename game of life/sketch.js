...
function setup() {
  // make the canvas the largest square you can
...
function mousePressed() {
  ...
  // and NEWS neighbour, if style is set to neighbours
...
function updateGrid() {
  // Need a second array so I dont mess with the grid while counting the neighbours
    ...
    for (let x = 0; x < GRID_SIZE; x++) {
      ...
      for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
          ...
          if (x + i >= 0 && x + 1 < GRID_SIZE && y + j >= 0 && y + j < GRID_SIZE)  {
